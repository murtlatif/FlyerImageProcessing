...
from .annotation_types import OCRAnnotation
from .google_cloud import google_cloud_client
from .google_cloud.annotation_response import (load_response_as_ocr_annotation,
                                               response_to_ocr_annotations,
...
def get_text_annotations(
    ...
    Returns:
        list[OCRAnnotation]: The list of annotations
    """
    if annotation_json_path:
        ocr_annotations = load_response_as_ocr_annotation(annotation_json_path)

    elif request_as_fallback and file_image_path is not None:
        if save_file is None:
            save_file = file_image_path
        save_file_name = get_file_name_without_ext(save_file)
        ocr_annotations = request_text_annotations(file_image_path, save_file=save_file_name)
...
def request_text_annotations(file_image_path: str, save_file_name: str = None) -> list[OCRAnnotation]:
    """
